...
(function($){
	...
	function __check_key() {
		...
		$(document).on( 'click', function(e) {
			...
			if( e.target && e.target.id == 'yfit-license-details' ) {
				_license_details();
			}
		} );
	...
	function __do_after_register(e,f) {
		...
		if ( u(f) && u(f.success) === false ) {
			...
			message_alert( 'Invalid YouthFireIT.com registration key', 'error', false );
		}
	}

	function _add_license_details_button() {
		var btn = '<a href="javascript:void(0)" id="yfit-license-details" class="yfit-button-primary x-color">License details</a>';
		if ( $('#youthfireit-nav').length>0 ) {
			$('#youthfireit-nav span:last').after(btn);
			return false;	
		}
	...
	function _license_details() {
		...
		var settings = {
			...
			'plugin' : typeof yfit != 'undefined' ? 'youthfireit' : svx.slug,
		};
	...
	function make_control_options(e,f) {
		...
		$.each( dO, function(i,g) {
			...
			var tmplData = {
				val: i,
				name: g,
				sel: u(f.val)!==false?f.val:u(f.default)?f.default:[]
			};
