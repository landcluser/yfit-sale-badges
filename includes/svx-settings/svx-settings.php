...
if ( isset( $GLOBALS['svx'] ) && version_compare( $GLOBALS['svx'], '1.3.7' ) == 0 ) :
...
if ( !class_exists( 'SevenVXSettings' ) ) {
	...
	class SevenVXSettings {
		...
		private function init_hooks() {
			...
			$page = isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'youthfireit' ? true: false;
		...
		public function add_templates() {
			...
			<script type="text/template" id="tmpl-svx-license">
				<div class="svx-license">
					<h2><?php esc_html_e( 'Plugin not registered!', 'youthfireit' ); ?></h2>
					<p>
						{{ data.name }} <?php esc_html_e( 'license is not registered. Please register! Click Register now button to start.', 'youthfireit' ); ?><br />
						<?php esc_html_e( 'Need help? Visit this link', 'youthfireit' ); ?> &rarr; <a href="https://help.youthfireit.com/how-to-register" target="_blank"><?php esc_html_e( 'How to register?', 'youthfireit' ); ?></a>
					</p>
					<span id="svx-register" class="svx-button"><?php esc_html_e( 'Register now', 'youthfireit' ); ?></span>
				</div>
			</script>
			
			<script type="text/template" id="tmpl-svx-license-details">
				<div class="svx-license details">
					<span class="svx-preformatted-row"><?php esc_html_e( 'Connecting to server', 'youthfireit' ); ?>...</span>
					<hr />
					<span id="svx-ok-license-details" class="svx-button svx-button-primary"><?php esc_html_e( 'OK', 'youthfireit' ); ?></span>
					<a href="https://help.youthfireit.com/my-support-tickets/" class="svx-button" target="_blank"><?php esc_html_e( 'Remove license', 'youthfireit' ); ?></a>
					<a href="https://help.youthfireit.com/my-support-tickets/" class="svx-button" target="_blank"><?php esc_html_e( 'License details', 'youthfireit' ); ?></a>
					<a href="https://youthfireit.com/store/" class="svx-button" target="_blank"><?php esc_html_e( 'Extend support', 'youthfireit' ); ?></a>
									</div>
			</script>
			
			<script type="text/template" id="tmpl-svx-license-input">
				<input id="svx-register-key" name="svx-register-key" placeholder="Paste YouthFireIT.com registration key" />
				<span id="svx-register-confirm" class="svx-button svx-button-primary"><?php esc_html_e( 'Confirm key', 'youthfireit' ); ?></span>
				<a href="https://help.youthfireit.com/my-support-tickets/" id="svx-register-get-key" class="svx-button" target="_blank"><?php esc_html_e( 'Get key', 'youthfireit' ); ?></a>
			</script>
...
?>
								<a href="javascript:void(0)" id="yfit-license-details" class="svx-button x-color">
									<?php esc_html_e( 'License details', 'youthfireit' ); ?>
								</a>
<?php
							}
?>
			...
			<script type="text/template" id="tmpl-svx-settings">
				<div id="svx-settings-main-{{ data.id }}" data-id="{{ data.id }}">
					<div id="svx-settings-wrapper">
						{{{ data.settings }}}
                        <span id="save-alt" class="svx-button-primary"><?php esc_html_e( 'Save', 'youthfireit' ); ?></span>
					</div>
				</div>
			</script>
			...
			<script type="text/template" id="tmpl-svx-option-utility">
				<span id="svx-export" class="svx-button"><?php esc_html_e( 'Export', 'youthfireit' ); ?></span><span id="svx-import" class="svx-button"><?php esc_html_e( 'Import', 'youthfireit' ); ?></span><span id="svx-backup" class="svx-button"><?php esc_html_e( 'Backup', 'youthfireit' ); ?></span><span id="svx-restore" class="svx-button"><?php esc_html_e( 'Restore', 'youthfireit' ); ?></span><span id="svx-reset" class="svx-button"><?php esc_html_e( 'Reset', 'youthfireit' ); ?></span>
			</script>
			...
			<script type="text/template" id="tmpl-svx-option-file">
				<input id="{{ data.eid }}" data-option="{{ data.id }}" class="svx-change{{ data.class }}" name="{{ data.name }}" type="text"<# if ( data.val ) { #> value="{{ data.val }}"<# } else { #><# } #> /> <span class="svx-button svx-file-add"><?php esc_html_e( 'Add +', 'youthfireit' ); ?></span>
			</script>
			...
			<script type="text/template" id="tmpl-svx-option-include">
				<span class="svx-button svx-include"><?php esc_html_e( 'Configure', 'youthfireit' ); ?></span>
			</script>
			...
			<script type="text/template" id="tmpl-svx-option-list">
				...
				<span class="svx-button-primary svx-option-list-add" data-id="{{ data.id }}"><?php esc_html_e( 'Add Item +', 'youthfireit' ); ?></span>
			</script>
			...
			<script type="text/template" id="tmpl-svx-option-list-select">
				...
				{{{ data.selects }}} <span class="svx-button-primary svx-option-list-select-add" data-id="{{ data.id }}"><?php esc_html_e( 'Add Item +', 'youthfireit' ); ?></span>
			</script>
				...
				<div id="svx-include-customizer" data-id="{{ data.id }}">
					<div class="svx-include-customizer-wrapper">
						<div class="svx-include-customizer-header">
							...
							<h2><?php esc_html_e( 'Include/Exclude Manager', 'youthfireit' ); ?></h2>
							<span id="svx-exclude-toggle" class="<# if ( data.selected == 'OUT' ) { #>svx-button-primary<# } else { #>svx-button<# } #>"><?php esc_html_e( 'Exclude', 'youthfireit' ); ?></span>
							<span id="svx-include-toggle" class="<# if ( data.selected == 'IN' ) { #>svx-button-primary<# } else { #>svx-button<# } #>"><?php esc_html_e( 'Include', 'youthfireit' ); ?></span>
						</div>
				...
				<div id="svx-customizer" data-id="{{ data.id }}">
					<div class="svx-customizer-wrapper">
						<div class="svx-customizer-header">
							...
							<h2><?php esc_html_e( 'Terms Manager', 'youthfireit' ); ?></h2>
							<# if ( data.type !== 'orderby' && data.type !== 'vendor' && data.type !== 'instock' && data.type !== 'range' ) { #>
								...
								<# }
									...
									<span id="svx-customizer-custom-order" class="<# if ( data.order == 'true' ) { #>svx-button-primary<# } else { #>svx-button<# } #>"><?php esc_html_e( 'Custom Order', 'youthfireit' ); ?></span>
								<# } #>
							...
							<div class="svx-customizer-style">
								<div id="svx-special-options">
									<span class="svx-special-option">
										<label><?php esc_html_e( 'Type', 'youthfireit' ); ?></label>
										<select class="svx-terms-style-change" data-option="type">
											<option value=""<# if ( data.style == '' ) { #> selected="selected"<# } #>><?php esc_html_e( 'None', 'youthfireit' ); ?></option>
											<option value="text"<# if ( data.style == 'text' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Plain Text', 'youthfireit' ); ?></option>
											<option value="color"<# if ( data.style == 'color' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Color', 'youthfireit' ); ?></option>
											<option value="image"<# if ( data.style == 'image' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Thumbnail', 'youthfireit' ); ?></option>
											<option value="selectbox"<# if ( data.style == 'selectbox' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Select Box', 'youthfireit' ); ?></option>
											<# if ( data.type.substr(0,5) !== 'ivpa_' ) { #>
												<option value="system"<# if ( data.style == 'system' ) { #> selected="selected"<# } #>><?php esc_html_e( 'System Select', 'youthfireit' ); ?></option>
												<option value="selectize"<# if ( data.style == 'selectize' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Live Select', 'youthfireit' ); ?></option>
											<# } #>	
											<option value="html"<# if ( data.style == 'html' ) { #> selected="selected"<# } #>>HTML</option>
											<# if ( data.type == 'ivpa_custom' ) { #>
												<option value="input"<# if ( data.style == 'input' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Input Field', 'youthfireit' ); ?></option>
												<option value="checkbox"<# if ( data.style == 'checkbox' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Checkbox', 'youthfireit' ); ?></option>
												<option value="textarea"<# if ( data.style == 'textarea' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Textarea', 'youthfireit' ); ?></option>
												<option value="system"<# if ( data.style == 'system' ) { #> selected="selected"<# } #>><?php esc_html_e( 'System Select', 'youthfireit' ); ?></option>
											<# } #>
										</select>
									</span>
			...
			<script type="text/template" id="tmpl-svx-customizer-style-text">
				<span class="svx-special-option">
					<label><?php esc_html_e( 'Style', 'youthfireit' ); ?></label>
					<select class="svx-terms-style-change" data-option="style">
						<option value="border"<# if ( data.border == 'round' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Border', 'youthfireit' ); ?></option>
						<option value="background"<# if ( data.style == 'background' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Background', 'youthfireit' ); ?></option>
						<option value="round"<# if ( data.style == 'round' ) { #> selected="selected"<# } #>><?php esc_html_e( 'Round', 'youthfireit' ); ?></option>
					</select>
				</span>
				<span class="svx-special-option">
					<label><?php esc_html_e( 'Normal', 'youthfireit' ); ?></label>
					<input type="text" class="svx-terms-color svx-terms-style-change" data-option="normal"<# if ( data.normal ) { #> value="{{ data.normal }}"<# } #> />
				</span>
				<span class="svx-special-option">
					<label><?php esc_html_e( 'Active', 'youthfireit' ); ?></label>
					<input type="text" class="svx-terms-color svx-terms-style-change" data-option="active"<# if ( data.active ) { #> value="{{ data.active }}"<# } #> />
				</span>
				<span class="svx-special-option">
					<label><?php esc_html_e( 'Disabled', 'youthfireit' ); ?></label>
					<input type="text" class="svx-terms-color svx-terms-style-change" data-option="disabled"<# if ( data.disabled ) { #> value="{{ data.disabled }}"<# } #> />
				</span>
				<span class="svx-special-option">
					<label><?php esc_html_e( 'Out of stock', 'youthfireit' ); ?></label>
			...
			<script type="text/template" id="tmpl-svx-customizer-style-swatch">
				<span class="svx-special-option">
					<label><?php esc_html_e( 'Show labels', 'youthfireit' ); ?></label>
				...
				<span class="svx-special-option svx-special-option-sm">
					<label><?php esc_html_e( 'Swatch size', 'youthfireit' ); ?></label>
				...
				<div class="svx-terms-list-item" data-id="{{ data.id }}" data-slug="{{ data.slug }}">
					...
					<div class="svx-term-options-holder">
						<div class="svx-term-option">
							<label><?php esc_html_e( 'Name', 'youthfireit' ); ?></label>
							<input type="text" class="svx-terms-change" name="name" />
						</div>

						<# if ( data.type == 'meta' || data.type == 'meta_range' ) { #>
							<div class="svx-term-option">
								<label><?php esc_html_e( 'Meta value', 'youthfireit' ); ?></label>
						...
						<# if ( data.style !== 'text' && data.style !== 'selectbox' && data.style !== 'system' && data.style !== 'selectize' ) { #>
							<div class="svx-term-option">
								<label><?php esc_html_e( 'Value', 'youthfireit' ); ?></label>
								<# if ( data.style !== 'html' ) { #>
						...
						<# } #>
 						...
 						<# if ( data.type == 'price' ) { #>
							<div class="svx-term-option">
								<label><?php esc_html_e( 'Min', 'youthfireit' ); ?></label>
								<input type="number" class="svx-terms-change" name="min" />
								<label><?php esc_html_e( 'Max', 'youthfireit' ); ?></label>
						...
						<# if ( data.type == 'per_page' ) { #>
							<div class="svx-term-option">
								<label><?php esc_html_e( 'Count', 'youthfireit' ); ?></label>
						...
						<# if ( data.type !== 'range' && data.type !== 'meta_range' && data.style !== 'system' && data.style !== 'selectize' ) { #>
							<div class="svx-term-option">
								<label><?php esc_html_e( 'Tooltip', 'youthfireit' ); ?></label>
					...
					<div class="svx-term-options-holder">
						...
						<# if ( data.style !== 'text' && data.style !== 'selectbox' ) { #>
							<div class="svx-term-option">
								...
								<# } #>
								<# if ( data.style == 'image' ) { #>
									<span class="svx-button svx-terms-image-add"><?php esc_html_e( 'Add Image +', 'youthfireit' ); ?></span>
								<# } #>
							</div>
						<# } #>
						...
						<# if ( data.type == 'ivpa_custom' ) { #>
							<div class="svx-term-option">
								<label><?php esc_html_e( 'Price', 'youthfireit' ); ?></label>
		...
		public function get_real_key( $plugin = 'youthfireit' ) {
			if ( class_exists( 'X_for_WooCommerce' ) ) {
				$plugin = 'youthfireit';
			}

			$code = get_option( 'yfit_key_' . $plugin );

			return $code === false ? 'false' : $code;
		}

		public function get_key( $plugin = 'youthfireit' ) {
			if ( class_exists( 'X_for_WooCommerce' ) ) {
				$plugin = 'youthfireit';
			}

			return get_option( 'yfit_key_' . $plugin ) === false ? 'false' : 'true';
		}

		public function _filter_get() {

			$key = isset( $_POST['svx']['settings'] ) ? sanitize_text_field( $_POST['svx']['settings'] ) : false;
